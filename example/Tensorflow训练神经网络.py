...
from example.mnist_test import mnist, x, y_

INPUT_NODE = 784  # 输入层的节点数。对于MNIST数据集，这个就等于图片的像素
OUTPUT_NODE = 10  # 输出层的节点数。这个等于类别的数目。区分0~9十个数字
...
# 通过ReLU函数实现去线性化。在这个函数中也支持传入用于计算参数平均值的类，
# 这样方便在测试时使用滑动平均模型
def inference(input_tensor, avg_class, weights1, biases1, weights2, biases2):
    ...
    if avg_class == None:
        ...
        # 计算输出层的向前在传播结果。因为在计算损失函时会一并计算softmax函数，
        # 所以这里不需要加入激活函数。而且不加入softmax不会影响预测结果。因为预测时
        # 使用的是不同类别对应节点输出值的相对大小有没有softmax层对最后分类结果的
        # 计算没有影响。于是正在计算整个神经网络的向前传播时可以不加入最后的softmax层。
...
def train(mnist):
    ...
    # 给定滑动平均衰减率和训练轮数打的变量，初始化滑动平均类。可以加快训练早期变量的更新速度
    ...
    # GraphKey.TRAINABLES中的元素。这个集合的元素就是所有没有指定trainable=False的参数
    global variable_averages_op
    ...
    global average_y
    average_y = inference(
        x, variable_averages, weights1, biases1, weight2, biases2
    )

    # 计算交叉熵作为刻画预测值和真实值之间距离的损失函数。这里使用了Tensorflow中提供的
    # sparse_soft_max_cross_entropy_with_logits函数来计算交叉熵。当分类问题只有
    # 一个正确答案时，可以使用这个函数来加速交叉熵的计算。MNIST问题的图片中只包含
    # 0~9中的一个数字，所以可以使用这个函数来计算交叉熵损失。这个函数的第一个
    # 参数是神经网络不包括softmax层的前向传播结果，第二个是训练数据的正确答案。因为
    # 标准答案是一个长度为10的一维数组，而该函数提供的是一个正确答案的数字，所以需要
    ...
    learning_rate = tf.train.exponential_decay(
        ...
        global_step,  # 当前的迭代轮数
        mnist.train.num.examples / BATCH_SIZE,  # 过完所有的训练数据所需要的迭代次数
    ...
    # tf.control_denpendencies和tf.group两种机制。下面两行程序和
...
# 第一个维度中进行，也就是说，只在每一行选取最大值对应的下标。于是得到觉得结果是一个
# 长度为batch的一维数组，这个一维数组中的值就表示了每一个样例对用的数字识别结果。
...
with tf.Session() as sess:
    ...
    for i in range(TRAINING_STEPS):
        ...
        if i % 1000 == 0:
            # 计算滑动平均模型在验证数据及上的结果。因为MNIST数据集比较小，所以一次
            # 可以处理所有的验证数据。为了计算方便，本样例程序没有将验证数据划分为
            # 更小的batch。当神经网络模型比较复杂或者验证数据较大时，太大的batch
